 ...
 *
 * Copyright (C) 2025 INGEINT <https://www.ingeint.com> and contributors (see README.md file).
 */

package org.idempiere.prueba.test.assertion;
...
public final class Annotations {
	...
	private static void assertAnnotations(String fieldOrClass, List<Annotation> actualAnnotations, List<Class<?>> expectedAnnotation, String parameterName, Object parameterValue) {
		...
		expectedAnnotation.forEach(clazz -> {
			List<Annotation> annotations = actualAnnotations.stream().filter(annotation -> annotation.annotationType().isAssignableFrom(clazz)).collect(Collectors.toList());
			...
			if (parameterName != null) {
				...
				annotations.forEach(annotation -> {
					...
					List<Method> methods = Arrays.asList(annotation.annotationType().getDeclaredMethods()).stream().filter(method -> method.getName().equals(parameterName))
	...
	public static void assertFieldAnnotation(Class<?> classUnderTest, String fieldUnderTest, Class<?>... expectedAnnotation) {
		try {
			assertAnnotations(fieldUnderTest, Arrays.asList(classUnderTest.getDeclaredField(fieldUnderTest).getAnnotations()), Arrays.asList(expectedAnnotation), null, null);
	...
	public static void assertClassAnnotationParameter(Class<?> classUnderTest, Class<?> expectedAnnotation, String parameterName, Object expectedParameterValue) {
		assertAnnotations(classUnderTest.toString(), Arrays.asList(classUnderTest.getAnnotations()), Arrays.asList(expectedAnnotation), parameterName, expectedParameterValue);
	}
	...
	public static void assertMethodAnnotationParameter(Class<?> classUnderTest, String methodUnderTest, Class<?> expectedAnnotation, String parameterName, Object expectedParameterValue) {
		try {
			assertAnnotations(methodUnderTest, Arrays.asList(classUnderTest.getDeclaredMethod(methodUnderTest).getAnnotations()), Arrays.asList(expectedAnnotation), parameterName,
	...
	public static void assertFieldAnnotationParameter(Class<?> classUnderTest, String fieldUnderTest, Class<?> expectedAnnotation, String parameterName, Object expectedParameterValue) {
		try {
			assertAnnotations(fieldUnderTest, Arrays.asList(classUnderTest.getDeclaredField(fieldUnderTest).getAnnotations()), Arrays.asList(expectedAnnotation), parameterName,
					expectedParameterValue);
		} catch (NoSuchFieldException e) {
			throw new AssertionError(e);
		}
