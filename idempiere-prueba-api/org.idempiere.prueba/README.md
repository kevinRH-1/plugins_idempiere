...
- New callout
    ...
    * Example: org.idempiere.prueba.callout.CStringFormat

- New process
    * Name: PName
    * Package: org.idempiere.prueba.process
    * Example: org.idempiere.prueba.process.PGenerateWithholding

- New form
    * Name: FName
    * Package: org.idempiere.prueba.form
    * Example: org.idempiere.prueba.form.FMultiPayment

- New event
    * Name: EName
    * Package: org.idempiere.prueba.event
    * Example: org.idempiere.prueba.event.EAfterCompleteInvoice

- New model (extends class X)
    * Name: MName
    * Package: org.idempiere.prueba.model
    * Example: org.idempiere.prueba.model.MTableExample

## Folder estructure

```
    org.idempiere.prueba
        |_.settings
...
- New callout
    * Create callout in package `org.idempiere.prueba.callout`, extends from `CustomCallout`
    * Annotate it with the `@Callout` annotation at class level

- New process
    * Create process in package `org.idempiere.prueba.process`, extends from `CustomProcess`
    * Annotate it with the `@Process` annotation at class level

- New form
    * Create form in package `org.idempiere.prueba.form`, extends from `CustomForm`
    * Annotate it with the `@Form` annotation at class level

- New event
    * Create event in package `org.idempiere.prueba.event`, extends from `CustomEvent`
    * Annotate it with the `@EventTopicDelegate` annotation at class level

- New model (extends form class X)
    * Create model in package `org.idempiere.prueba.model`, extends class `X`. Example: `X_TL_TableExample -> MTableExample`
